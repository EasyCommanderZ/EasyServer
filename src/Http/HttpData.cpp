...
HttpData::HttpData(SP_Channel channel, int connfd) :
...
void HttpData::handleRead() {
    std::uint32_t &events = _channel->getEvents();
    do {
        bool zero = false;
...
void HttpData::handleError(int fd, int err_num, std::string short_msg) {
    LOG_ERROR("HttpData handleError, fd : %s, ereCode : %d, msg : %s\n", fd, err_num, short_msg.c_str());
    _response.setCode(400);
    _response.handleErrorResponse(_outBuffer);
}
